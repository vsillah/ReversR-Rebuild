...
(async () => {
 ...
 await prodPage.getByTestId('cad-upload-locked-status').waitFor();
 assert.equal(await prodPage.getByRole('button',{name:'Upload unavailable: operator access required'}).count(),0);
