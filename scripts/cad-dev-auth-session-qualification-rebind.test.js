...
const packet = JSON.parse(fs.readFileSync('docs/cad-dev-auth-session-qualification-rebind.json', 'utf8'));
const markdown = fs.readFileSync('docs/cad-dev-auth-session-qualification-rebind.md', 'utf8');
...
test('rebind packet binds one exact development target and UTC window', () => {
  ...
  assert.equal(packet.baseMainCommit, 'ab35e48cf5bdc8e5574e31eda7ff7ea433211a6e');
  assert.equal(packet.target.deploymentName, 'majestic-alligator-31');
  assert.equal(packet.target.kind, 'development');
  assert.equal(packet.window.startUtc, '2026-09-15T18:00:00Z');
  assert.equal(packet.window.endUtc, '2026-09-15T18:15:00Z');
...
test('binding is enabled only for the accepted one-run digest/window tuple', () => {
  ...
  assert.match(binding, /mode: 'cad-dev-auth-session-qualification-rebind-1800z'/);
  assert.match(binding, /runId: 'cad-dev-auth-session-qualification-1800z'/);
