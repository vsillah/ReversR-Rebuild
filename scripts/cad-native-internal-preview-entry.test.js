...
test('native Import opens the internal installed-app CAD renderer without unlocking production upload', () => {
  ...
  assert.match(importPanel, /cad-import-grid-preview/);
  assert.match(importPanel, /Choose CAD file to load/);
  assert.match(importPanel, /cad-import-viewfinder-controls/);
  assert.match(importPanel, /cad-import-viewfinder-dial/);
  assert.match(importPanel, /cad-import-viewfinder-zoom/);
  assert.match(importPanel, /const isDarkGrid = colors\.mode === 'dark'/);
  assert.match(importPanel, /const gridBackground = isDarkGrid/);
  assert.match(importPanel, /backgroundColor: gridBackground/);
  assert.match(importPanel, /const gridCenterGuide = isDarkGrid/);
  assert.match(importPanel, /const previewDialSurface = isDarkGrid/);
  assert.match(importPanel, /const previewZoomSurface = isDarkGrid/);
  assert.doesNotMatch(importPanel, /cad-import-grid-wave/);
  assert.doesNotMatch(importPanel, /cad-import-grid-object/);
  assert.doesNotMatch(importPanel, /cad-choose-file/);
