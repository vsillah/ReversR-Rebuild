// Checks only the public source-only accepted projection packet. Never reads private receipts.
const fs = require('node:fs');
const path = require('node:path');
const { createHash } = require('node:crypto');
const { isDeepStrictEqual } = require('node:util');
const { plainData } = require('../offline/cad-auth-command-card-source/preparation');
const { PACKET: PARENT_PACKET, checkStopRepairPacket: checkParent } =
  require('./cad-auth-provenance-review-stop-repair-checker');
const {
  SOURCE_MERGE_COMMIT,
  projectionPreparation,
  checkProjectionPreparation,
...
function expectedPacket(readSource = read) {
  ...
  if (!checkParent(parent, { readSource }).ok) throw Error('INVALID_STOP_REPAIR_PARENT');
  return {
    schemaVersion: 1,
    packet: 'cad-auth-accepted-provenance-projection-v1',
    sourceOnly: true,
...
function checkProjectionPacket(packet, { readSource = read } = {}) {
  ...
  try {
    ok = packet !== null && typeof packet === 'object' && !Array.isArray(packet)
      ...
      && checkProjectionPreparation(packet.preparation).ok;
  } catch { /* sanitized */ }
  return {
    ...checkProjectionPreparation(null),
    ok,
