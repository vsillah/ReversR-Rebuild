...
const { PACKET, SOURCES, checkIntake } = require('./cad-auth-receipt-intake-template-checker');
...
test('source contract validates with eight missing bindings and zero execution authority', () => {
  ...
  assert.equal(p.parent.historicalCardReusable, false);
  assert.equal(p.parent.priorApprovalReusable, false);
  assert.equal(checkIntake(p).ok, true);
  assert.equal(checkDisabledIntake(p.preparation).ok, true);
  closed(checkIntake(p));
...
test('custody roles and independent review requirements stay unbound', () => {
  const binding = disabledIntake();
  assert.equal(binding.bindingStatus, 'RECEIPT_INTAKE_TEMPLATE_UNBOUND_SOURCE_ONLY');
...
test('malformed, private, cyclic, accessor and proxy inputs fail without evaluation or disclosure', () => {
  ...
  for (const input of [null, [], 'PRIVATE_SENTINEL', undefined, NaN, new Date(),
    ...
    for (const check of [checkIntake, checkDisabledIntake]) {
...
test('every receipt promotion and populated evidence slot fails closed', () => {
  for (const name of REQUIRED_LIVE_BINDINGS) {
    const receipt = disabledIntake().runtimeBindingReceipts[name];
    for (const field of Object.keys(receipt)) {
      const p = packet();
      p.preparation.runtimeBindingReceipts[name][field] = typeof receipt[field] === 'boolean' ? true : 'PRIVATE_SENTINEL';
      assert.equal(checkIntake(p).ok, false);
      assert.equal(checkDisabledIntake(p.preparation).ok, false);
    }
    for (const field of Object.keys(receipt.evidence)) {
      ...
      const result = checkIntake(p); assert.equal(result.ok, false); closed(result);
    }
  }
  const p = packet();
...
test('live commands, gates, counters, merge drift and extra fields cannot promote a card', () => {
  for (const [key, value] of Object.entries(disabledIntake())) {
  ...
  for (const key of Object.keys(disabledIntake().futureHumanGate)) {
  ...
  for (const mutate of [p => { p.parent.mergeCommit = 'drift'; },
    ...
    const p = packet(); mutate(p); assert.equal(checkIntake(p).ok, false);
  }
});
...
test('direct and transitive source drift fails closed', () => {
  for (const file of [...SOURCES, 'offline/cad-auth-live-collector-binding/guardedCollector.js',
    ...
    const result = checkIntake(packet(), { readSource: name => name === file
      ? Buffer.concat([read(name), Buffer.from('\n')]) : read(name) });
    assert.equal(result.ok, false); closed(result);
  }
  const result = checkIntake(packet(), { readSource: () => { throw Error('PRIVATE_SENTINEL'); } });
  assert.equal(result.ok, false); closed(result);
});
...
test('CLI rejects live, retry, collect, issuance and unknown modes without writing', () => {
  ...
  for (const args of [['--live'], ['--retry'], ['--collect'], ['live'], ['retry'], ['collect'],
    ...
    const result = spawnSync(process.execPath, ['scripts/cad-auth-receipt-intake-template-checker.js', ...args],
...
test('offline module has no I/O and no runtime source imports the preparation package', () => {
  assert.doesNotMatch(read('offline/cad-auth-receipt-intake-template/preparation.js').toString(),
  ...
  for (const dir of ['server', 'api', 'app', 'convex', 'components', 'hooks', 'utils', 'constants']) {
    for (const file of walk(dir)) assert.doesNotMatch(read(file).toString(), /cad-auth-receipt-intake-template/, file);
  }
});
...
test('nested hostile receipt data is rejected before hooks or source reads', () => {
  ...
  for (const hostile of [accessor, proxy, cyclic, revoked.proxy, hidden, { toJSON: hook },
    ...
    assert.equal(checkDisabledIntake(p.preparation).ok, false);
    const result = checkIntake(p, { readSource: hook });
    assert.equal(result.ok, false); closed(result);
  }
  assert.equal(calls, 0);
});
...
test('parent validation cannot be bypassed by rebinding a modified parent hash', () => {
  ...
  const parentFile = 'docs/cad-auth-sealed-card-custody-rebind.json';
  ...
  const result = checkIntake(p, { readSource: file => file === parentFile ? bytes : read(file) });
  assert.equal(result.ok, false); closed(result);
});
...
test('plausible receipt refs, digests, identities and targets remain rejected', () => {
  ...
  for (const [route, value] of cases) {
    ...
    for (const result of [checkIntake(p), checkDisabledIntake(p.preparation)]) {
      assert.equal(result.ok, false); closed(result);
...
test('intake requires all categories, provenance and independent custody review with empty slots', () => {
  const intake = disabledIntake().intake;
  assert.equal(intake.status, 'NOT_STARTED_SEPARATE_AUTHORITY_REQUIRED');
  assert.equal(intake.receiptIngestionImplemented, false);
  assert.deepEqual(Object.keys(intake.slots), REQUIRED_LIVE_BINDINGS);
  for (const [category, slot] of Object.entries(intake.slots)) {
    assert.equal(slot.categoryContract, 'runtimeBindingReceipts.' + category);
    assert.equal(slot.required, true);
    for (const [key, value] of Object.entries(slot)) {
      if (['status', 'required', 'categoryContract'].includes(key)) continue;
      assert.ok(value === null || value === false, key);
    }
  }
  for (const [key, value] of Object.entries(intake.prerequisites)) {
    assert.equal(value, key !== 'automaticPromotionAllowed');
  }
});

test('generator is deterministic and reads only repository source bindings', () => {
  const { expectedPacket } = require('./cad-auth-receipt-intake-template-checker');
  const requested = new Set();
  const generated = expectedPacket(file => {
  ...
  assert.deepEqual(expectedPacket(), generated);
  for (const source of SOURCES) assert.ok(requested.has(source));
});
